...
def process_bad_proportions(original_image, size, faces):
	...
	#fixed width, width is longer than height
	...
	#fixed width, width is shorter than height
	new_height = 3 * width / 2
	if(new_height < height):
		lost_area = width * (height - new_height)
		boxes.append(Crop(box=(0, 0, width, new_height), score=lost_area))
		boxes.append(Crop(box=(0, height - new_height, width, height), score=lost_area))
	...
	#fixed height, width is shorter than height
	new_width = 2 * height / 3  
	if(new_width < width):
	...
	#fixed height, width is longer than height
